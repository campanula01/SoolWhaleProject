		...
		<div class="wrap_center wrap_myinfo">
			<div class="d-flex flex-column flex-shrink-0 p-3 text-bg-dark"
				...
				<a href="/"
					...
					<img src="/resources/img/common/user_img.png" style="width: 50px; height: 50px"> <span class="fs-4">술고래</span>
				</a>
				<hr>
                        ...
                        <c:choose>
                            <c:when test="${not empty inquiryList}">
                                <c:forEach var="inquiry" items="${inquiryList}" varStatus="status">
                                        <form id="${inquiry.inquiryNum}_form">
                                            <input type="hidden" id="projectNum" name="projectNum" value="${inquiry.inquiryNum}">
                                        </form>
                                        <tr>
                                            <td>${inquiry.inquiryNum}</td>
                                            <td>${inquiry.cat}</td>
                                            <td>${inquiry.userType}</td>
                                            <td>${inquiry.inquiryTitle}</td>
                                            <td>${inquiry.inquiryQ}</td>
                                            <td>${inquiry.inquiryDate}</td>
                                            <td><button type="button" class="acceptBtn" data-project-num="${inquiry.inquiryNum}">승인</button></td>
                                            <td><button type="button" class="refuseBtn" data-project-num="${inquiry.inquiryNum}">반려</button></td>
                                        </tr>
                                </c:forEach>
                            </c:when>
                        </c:choose>
